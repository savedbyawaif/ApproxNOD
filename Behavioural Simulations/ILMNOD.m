...
function final = aILM(x,y)
    input1 = dec2bin(x, 8);
    input2 = dec2bin(y, 8);
   
    input_1nod = ApproxNOD(bin2dec(input1));
    
    input_peout1 = PriorityEncoder8(input_1nod);
    
    input_2nod = ApproxNOD(bin2dec(input2));
...
% approximate NOD
function binaryANOD = ApproxNOD(binaryNumber)
    bitanod = zeros(1,9);
    ...
    binaryANOD = dec2bin(bin2dec(num2str(bitanod)), 9);
end
